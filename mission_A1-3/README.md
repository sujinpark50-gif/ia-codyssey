...
기록은 있는데 글쓰기가 어려운 사람을 위한 AI 카피라이터입니다.
...
```
 ...
 - api/index.py: AI 글 다듬기 API (vercel.json이 /api 경로를 이 파일로 연결)
 - images/: README에 사용하는 화면 캡처
...
사용자가 직접 작성한 메모 또는 글의 초안 (최대 500자)
...
원문의 의미와 사실관계를 유지하면서 자연스럽게 다듬어진 글
 ...
 - 실패 처리
입력값이 비어 있으면 API를 호출하지 않고 입력 안내 메시지를 표시합니다.
입력이 500자를 넘으면 서버가 HTTP 413으로 거절합니다.
API 요청 중에는 버튼을 비활성화하여 중복 요청을 방지합니다.
API 호출에 실패하면 오류 메시지를 표시하고 10초 후 다시 시도할 수 있습니다.

 - 체험 횟수

변환 한 번이 곧 API 비용이므로 체험은 1회로 제한합니다.
변환에 성공하면 버튼이 잠기고 무료 베타 신청을 안내합니다.
변환에 실패한 경우에는 잠그지 않고 10초 후 재시도할 수 있게 합니다.
